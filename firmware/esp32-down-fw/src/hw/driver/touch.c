...
bool touchInit(void)
{
  ...
  if (ret == true)
  {
    touch_width  = cst816tGetWidth();
    touch_height = cst816tGetHeight();
    is_init = true;
    is_enable = true;
  }
...
bool touchGetInfo(touch_info_t *p_info)
{
  ...
  ret = cst816tGetInfo(&ts_info);
  if (ret == true)
  {
    p_info->count = ts_info.count;
    for (int i=0; i<ts_info.count; i++)
    {
      p_info->point[i].event = ts_info.point[i].id;
      p_info->point[i].id    = ts_info.point[i].num;
  ...
  if (args->argc == 2 && args->isStr(0, "get") && args->isStr(1, "info"))
  {
    ...
    while(cliKeepLoop())
    {
      ...
      if (touchGetInfo(&info) == true)
      {
        ...
        {
          cliPrintf(" - ");
          cliPrintf("id=%d evt=%2d x=%3d y=%3d w=%3d %s", 
            info.point[i].id,      
            info.point[i].event,      
            info.point[i].x, 
            info.point[i].y, 
            info.point[i].w,
            cst816tGetGestureStr(info.point[i].event)
            );
        }
