  ...
  if (args->argc == 1 && args->isStr(0, "test") == true)
  {
    ...
    while(cliKeepLoop())
    {
      ...
      if (lcdDrawAvailable() == true)
      {
        ...
        lcdPrintf(0,50+16*1, white, "%d fps", lcdGetFps());
        lcdPrintf(0,50+16*2, white, "%d ms" , lcdGetFpsTime());
        lcdPrintf(0,50+16*3, white, "%d ms free" , lcdGetFpsTime() - lcdGetDrawTime());
        lcdPrintfResize(LCD_WIDTH-30, 50+16, white, 24, "%02d", cnt%100);
        lcdPrintfResize(LCD_WIDTH-40, 50+40, white, 32, "%02d", cnt%100);
        cnt++;
